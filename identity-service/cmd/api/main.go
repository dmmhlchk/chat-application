...
import (
	...
	"identity-service/internal/infrastructure/persistence/redis"
...
func main() {
	...
	defer redisdb.Close()
	otpRepo := redis.NewOTPCacheRepository(redisdb)

	// ----------------------------------------------------------------------
	//  			INFRASTRUCTURE: CRYPTO & SMS PROVIDER INITIALIZATION
	// ----------------------------------------------------------------------

	jwtGen := security.NewJWTGenerator(jwtSecret)
	otpGen := security.NewSecureOTPGenerator()
	// otpSender := security.NewSecureOTPGenerator()
	passwordHasher := security.NewBcryptPasswordHasher()
	// dummySMS := sms.NewDummySMSProvider()

	// ----------------------------------------------------------------------
	//  			APPLICATION: USE CASE INITIALIZATION
	// ----------------------------------------------------------------------

	userSignUpReqUC := usecase.NewSignUpRequest(userRepo, dummySMS, otpGen, otpRepo)
	userSignUpConfUC := usecase.NewSignUpConfirm(userRepo, otpRepo, passwordHasher)
	userSignInUC := usecase.NewSignIn(userRepo, sessionRepo, passwordHasher, jwtGen)
	userDeletionUC := usecase.NewUserDelete(userRepo, sessionRepo, passwordHasher)
	sessionTerminationUC := usecase.NewTerminateSession(sessionRepo, jwtGen)
	sessioListUC := usecase.NewSessionList(sessionRepo)
	passwordChangeUC := usecase.NewChangePassword(userRepo, passwordHasher)
